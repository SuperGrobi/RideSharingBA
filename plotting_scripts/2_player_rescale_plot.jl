...
p1 = plot(xlabel=L"b", ylabel=L"sharing regime width $\theta$", title="unscaled",
    xlims=(0, :auto),
    ylims=(0,2π+0.05),
    framestyle=:box,
    xticks=(0:1:4, latexstring.(0:1:4)),
    yticks=(0:π/2:2π, [L"0", L"\frac{\pi}{2}", L"\pi", L"\frac{3\pi}{2}", L"2\pi"]))
p2 = plot(xlabel=L"\frac{b}{a}", title="rescaled",
    xlims=(0,10),
    ylims=(0,2π+0.05),
    framestyle=:box,
    xticks=(0:2:10, latexstring.(0:2:10)),
...
vline!(p2, [π/2], label="", color=:black, alpha=0.4, width=3, linestyle=:dash)
fig = plot(p1,p2, size=(1000,400))
savefig(fig, "../writing/rescalability_no_inc.pdf")
